struct BExitGraphNode {
	...
	LinkedList<BExitGraphNode*> in;
	LinkedList<BExitGraphNode*> out;
	...
	BExitGraphNode(int ix, int iy, int iw, int ih, int t = 0, long o = 0) {
		branch = 0;
		brasec = false;
		branch2 = 0;
		brasec2 = false;
	...
	void paint(HDC target) {
		...
		LLNode<BExitGraphNode*>*cur = out.head;
		i = 0;
		while (cur) {
