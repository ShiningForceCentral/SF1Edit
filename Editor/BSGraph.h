#include "BSGraphNode.h"

#define NUM_BS_COLORS	0
Triple<String, COLORREF, COLORREF> BSColors[] = {
...
struct BSGraph {
	LinkedList<BSGraphNode*> nodes;
	BSGraphNode * sel;
	int selplace;
	bool scroll;
	bool drag;
	bool dragsel;
	LinkedList<BSGraphNode*> selgroup;
	...
	BSGraph(int ix, int iy, int iw, int ih) {
		x = ix;
		y = iy;
		w = iw;
		h = ih;
	...
	void copy() {
		...
		LLNode<BSGraphNode*>* cur = selgroup.head;
		LLNode<BSGraphNode*>* cur2;
		LLNode<BSGraphNode*>* cur3;

		sprintf(out, "BSNodes - %d\r\n", selgroup.size);
		...
		while (cur) {
			type = cur->data->type - nodeTypesBS;
	...
	void paste() {
		...
		for (int q = 0; q < t; q++) {
			...
			add = new BSGraphNode(x, y, 80, 80, ot);
			nodes.add(add);
			selgroup.append(add);
			if (!q)sel = add;
	...
	void buildList() {
		choices.clean();
		for (int i = 0; i<NumBSNodeTypes; i++) {
			if (nodeTypesBS[i].shape == select[mode])choices.append(i);
		}
	}
	...
	BSGraphNode * findMessage(int mess, BSGraphNode * start = 0) {
		BSGraphNode * ret = 0;
		BSGraphNode * cur = start;
	...
	BSGraphNode * findEvent(int event, BSGraphNode * start = 0) {
		BSGraphNode * ret = 0;
		BSGraphNode * cur = start;
		if (!cur)cur = nodes.tail->data;
	...
	BSGraphNode * findRegion(int region, BSGraphNode * start = 0) {
		BSGraphNode * ret = 0;
		BSGraphNode * cur = start;
		if (!cur)cur = nodes.tail->data;
	...
	BSGraphNode * findExit(int exit, BSGraphNode * start = 0, bool first = false) {
		BSGraphNode * ret = 0;
		BSGraphNode * cur = start;
		if (!cur)cur = nodes.tail->data;
	...
	void paint(HDC target) {
		...
		while (curi&&i<18) {
			...
			for (int dc = 0; dc < NUM_BS_COLORS; dc++) {
				if (!strcmp(BSColors[dc].elemA, nodeTypesBS[curi->data].name)) {
					old = (HPEN)SelectObject(target, CreatePen(PS_SOLID, 1, BSColors[dc].elemB));
					SelectObject(target, CreateSolidBrush(BSColors[dc].elemC));
					break;
				}
			}
			...
			TextOut(target, x - 233, y + 32 + 20 * i, nodeTypesBS[curi->data].name, strlen(nodeTypesBS[curi->data].name));

			if (strlen(nodeTypesBS[curi->data].helptext)>0) {
		...
		LLNode<BSGraphNode*>* cur = nodes.head;
		while (cur) {
			cur->data->x -= vx;
			cur->data->y -= vy;
			cur = cur->next;
		...
		while (cur) {
			...
			if (cur->data->selected || selgroup.find(cur->data)) {
				...
				for (int dc = 0; dc < NUM_BS_COLORS; dc++) {
					if (!strcmp(BSColors[dc].elemA, cur->data->type->name)) {
						SelectObject(gdc, CreatePen(PS_SOLID, 2, BSColors[dc].elemB));
						SelectObject(gdc, CreateSolidBrush(BSColors[dc].elemC));
						break;
					}
				}
			}
	...
	void addNode(int x, int y, int t = 0, long o = 0) {
		if (x<0)x = 0;
		if (y<0)y = 0;
		nodes.add(new BSGraphNode(x, y, 80, 80, t, o));
	}
		...
		while (curi&&i<18) {
			if (strlen(nodeTypesBS[curi->data].helptext)>0)
				if (px >= x - 42 && px <= x - 22 && py >= y + 30 + 20 * i&&py <= y + 50 + 20 * i) {
					MessageBox(NULL, nodeTypesBS[curi->data].helptext, "Help", MB_OK);
					return;
				}
	...
	void search(int type) {
		...
		if (node) {
			...
			if (searchmulti && subselect[mode][0] != searchstart - 1 && (searchstart != 0 || subselect[mode][0] != NUM_BS - 1)) {
				subselect[mode][0]++;
				if (subselect[mode][0] == NUM_BS)subselect[mode][0] = 0;
			} else {
				searchstart = -1;
						...
						if (cur->data != sel && px >= cur->data->x&&px <= cur->data->x + cur->data->w&&py >= cur->data->y&&py <= cur->data->y + cur->data->h) {
							if (!sel->in.find(cur->data) && cur->data->type->shape != 2 && cur->data->type->shape != 3) {
								...
								if (MessageBox(hwnd, "Connect end to target?", "Confirm", MB_YESNO) == IDYES) {
									...
									LLNode<BSGraphNode*>*find;
									LLNode<BSGraphNode*>*ins;
	...
	void connect(BSGraphNode * a, BSGraphNode * b, bool prev = false) {
	...
	void removeIns() {
		if (!sel)return;
		LLNode<BSGraphNode*>*cur = sel->in.head;
		while (cur) {
	...
	void removeOuts() {
		if (!sel)return;
		LLNode<BSGraphNode*>*cur = sel->out.head;
		while (cur) {
	...
	void remove() {
		...
		if (selgroup.size) {
			LLNode<BSGraphNode*>*cur2 = selgroup.head;
