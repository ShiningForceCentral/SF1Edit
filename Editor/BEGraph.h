#include "BEGraphNode.h"

#define NUM_BE_COLORS	2
Triple<String, COLORREF, COLORREF> BEColors[] = {
	Triple<String, COLORREF, COLORREF>("Set Event Flag" ,RGB(0,0,192),RGB(128,128,255)),
	Triple<String, COLORREF, COLORREF>("Clear Event Flag" ,RGB(0,0,192),RGB(128,128,255))
};

struct BEGraph{
	LinkedList<BEGraphNode*> nodes;
	BEGraphNode * sel;
	int selplace;
	bool scroll;
	bool drag;
	bool dragsel;
	LinkedList<BEGraphNode*> selgroup;
	...
	BEGraph(int ix, int iy, int iw, int ih){
		x=ix;
		y=iy;
		w=iw;
		h=ih;
	...
	void copy() {
		...
		LLNode<BEGraphNode*>* cur = selgroup.head;
		LLNode<BEGraphNode*>* cur2;
		LLNode<BEGraphNode*>* cur3;

		sprintf(out, "BENodes - %d\r\n", selgroup.size);
		...
		while (cur) {
			type = cur->data->type - nodeTypesBE;
	...
	void paste() {
		...
		for (int q = 0; q < t; q++) {
			...
			add = new BEGraphNode(x, y, 80, 80, ot);
			nodes.add(add);
			selgroup.append(add);
			if (!q)sel = add;
	...
	void buildList(){
		choices.clean();
		for(int i=0;i<NumBENodeTypes;i++){
			if(nodeTypesBE[i].shape==select[mode])choices.append(i);
		}
	}
	...
	BEGraphNode * findMessage(int mess, BEGraphNode * start = 0){
		BEGraphNode * ret = 0;
		BEGraphNode * cur = start;
	...
	void paint(HDC target){
		...
		while(curi&&i<18){
			...
			for (int dc = 0; dc < NUM_BE_COLORS; dc++) {
				if (!strcmp(BEColors[dc].elemA, nodeTypesBE[curi->data].name)) {
					old = (HPEN)SelectObject(target, CreatePen(PS_SOLID, 1, BEColors[dc].elemB));
					SelectObject(target, CreateSolidBrush(BEColors[dc].elemC));
					break;
				}
			}
			...
			TextOut(target,x-233, y+32+20*i, nodeTypesBE[curi->data].name, strlen(nodeTypesBE[curi->data].name));

			if(strlen(nodeTypesBE[curi->data].helptext)>0){
		...
		LLNode<BEGraphNode*>* cur = nodes.head;
		while(cur){
			cur->data->x-=vx;
			cur->data->y-=vy;
			cur=cur->next;
		...
		while(cur){
			//if(cur->data->x>=0-cur->data->w && cur->data->x<=w && cur->data->y>=0-cur->data->h && cur->data->y<=h){
				if(cur->data->selected || selgroup.find(cur->data)){
					...
					for (int dc = 0; dc < NUM_BE_COLORS; dc++) {
						if (!strcmp(BEColors[dc].elemA, cur->data->type->name)) {
							SelectObject(gdc, CreatePen(PS_SOLID, 2, BEColors[dc].elemB));
							SelectObject(gdc, CreateSolidBrush(BEColors[dc].elemC));
							break;
						}
					}
				}
		...
		if(menu==2){
			...
			Rectangle(gdc,w/2-200+258,h/2-150+53,w/2-200+348,h/2-150+86);
	...
	void addNode(int x, int y, int t=0, long o=0){
		if(x<0)x=0;
		if(y<0)y=0;
		nodes.add(new BEGraphNode(x,y,80,80,t,o));
	}
...
void keydown(int key){
	...
	void click(int px, int py){
		...
		if(menu==2){
			...
			if(px>=w/2-200&&px<=w/2+200&&py>=h/2-150&&py<=h/2+150){
				...
				if(px>=w/2-200+258&&px<=w/2-200+348&&py>=h/2-150+53&&py<=h/2-150+86){
					search(0);
				}
		...
		while(curi&&i<18){
			...
			if(px>=x-42&&px<=x-22&&py>=y+30+20*i&&py<=y+50+20*i){
				MessageBox(NULL,nodeTypesBE[curi->data].helptext,"Help",MB_OK);
				return;
			}
	...
	void search(int type) {
		...
		if (node) {
			...
			if (searchmulti && subselect[mode][0] != searchstart - 1 && (searchstart != 0 || subselect[mode][0] != NUM_BE - 1)) {
				subselect[mode][0]++;
				if (subselect[mode][0] == NUM_BE)subselect[mode][0] = 0;
			} else {
				searchstart = -1;
	...
	void connect(BEGraphNode * a, BEGraphNode * b, bool prev = false){
	...
	void removeIns() {
		if (!sel)return;
		LLNode<BEGraphNode*>*cur = sel->in.head;
		while (cur) {
	...
	void removeOuts() {
		if (!sel)return;
		LLNode<BEGraphNode*>*cur = sel->out.head;
		while (cur) {
	...
	void remove(){
		...
		if (selgroup.size) {
			LLNode<BEGraphNode*>*cur2 = selgroup.head;
