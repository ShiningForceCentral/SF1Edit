struct BEGraphNode{
	...
	LinkedList<BEGraphNode*> in;
	LinkedList<BEGraphNode*> out;
	...
	BEGraphNode(int ix, int iy, int iw, int ih, int t=0, long o=0){
		branch = 0;
		brasec = false;
		branch2 = 0;
		brasec2 = false;
	...
	void paint(HDC target){
		...
		LLNode<BEGraphNode*>*cur = out.head;
		i=0;
		while(cur){
		...
		if (GraphicsLoaded && !strcmp(type->name, "Display Portrait")) {
			int m = args[0];
