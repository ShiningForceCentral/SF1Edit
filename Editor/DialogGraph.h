unsigned int GRAPHSIZE = 2048 * 8;

#include "NodeType.h"
#include "DialogGraphNode.h"

#define NUM_DIALOG_COLORS	10
Triple<String, COLORREF, COLORREF> DialogColors[] = {
	Triple<String, COLORREF, COLORREF>("Set Money",RGB(192,192,0),RGB(255,255,128)),
	Triple<String, COLORREF, COLORREF>("Add Money",RGB(192,192,0),RGB(255,255,128)),

	Triple<String, COLORREF, COLORREF>("Add to Force (Data Only)",RGB(0,192,0),RGB(128,255,128)),
	Triple<String, COLORREF, COLORREF>("Add to Force (Sound+MSG)",RGB(0,192,0),RGB(128,255,128)),
	Triple<String, COLORREF, COLORREF>("Add to Force (Standard)",RGB(0,192,0),RGB(128,255,128)),

	Triple<String, COLORREF, COLORREF>("Priest Menu" ,RGB(192,0,192),RGB(255,128,255)),
	Triple<String, COLORREF, COLORREF>("Display Shop",RGB(192,0,192),RGB(255,128,255)),
...
struct DialogGraph{
	LinkedList<DialogGraphNode*> nodes;
	DialogGraphNode * sel;
	int selplace;
	bool scroll;
	bool drag;
	bool dragsel;
	LinkedList<DialogGraphNode*> selgroup;
	...
	DialogGraph(int ix, int iy, int iw, int ih){
		x=ix;
		y=iy;
		w=iw;
		h=ih;
	...
	void copy() {
		...
		LLNode<DialogGraphNode*>* cur = selgroup.head;
		LLNode<DialogGraphNode*>* cur2;
		LLNode<DialogGraphNode*>* cur3;

		sprintf(out, "DGNodes - %d\r\n", selgroup.size);
		...
		while (cur) {
			type = cur->data->type - nodeTypesDialog;
	...
	void paste() {
		...
		for (int q = 0; q < t; q++) {
			...
			add = new DialogGraphNode(x, y, 80, 80, ot);
			nodes.add(add);
			selgroup.append(add);
			if (!q)sel = add;
	...
	void buildList(){
		choices.clean();
		for(int i=0;i<NumDialogNodeTypes;i++){
			if(nodeTypesDialog[i].shape==select[mode])choices.append(i);
		}
	}
	...
	DialogGraphNode * findMessage(int mess, DialogGraphNode * start = 0){
		DialogGraphNode * ret = 0;
		DialogGraphNode * cur = start;
	...
	DialogGraphNode * findEvent(int event, DialogGraphNode * start = 0) {
		DialogGraphNode * ret = 0;
		DialogGraphNode * cur = start;
		if (!cur)cur = nodes.tail->data;
	...
	DialogGraphNode * findAddMember(int member, DialogGraphNode * start = 0) {
		DialogGraphNode * ret = 0;
		DialogGraphNode * cur = start;
		if (!cur)cur = nodes.tail->data;

		if (searchmet) {
			if (!strcmp(cur->type->name, "Add to Force (Data Only)") || !strcmp(cur->type->name, "Add to Force (Standard)")) {
				if (cur->args[0] == member)return cur;
			}
			if (!strcmp(cur->type->name, "Add to Force (Sound+MSG)")) {
				if (cur->args[2] == member)return cur;
			}
		}
		...
		for (int i = 0; i<cur->out.size; i++) {
			ret = findAddMember(member, cur->out[i]->data);
			if (ret)return ret;
		}
		return ret;
	}

	DialogGraphNode * findDialogNumber(int num, DialogGraphNode * start = 0){
		DialogGraphNode * ret = 0;
		DialogGraphNode * cur = start;
		if(!cur)cur = nodes.tail->data;

		if(!strcmp(cur->type->name,"Check Dialog Number")){
			if(cur->args[0]==num)return cur;
		}

		for(int i=0;i<cur->out.size;i++){
			ret = findDialogNumber(num,cur->out[i]->data);
			if(ret)return ret;
		}
		return ret;
	}
	...
	void paint(HDC target){
		...
		while(curi&&i<18){
			...
			for (int dc = 0; dc < NUM_DIALOG_COLORS; dc++) {
				if (!strcmp(DialogColors[dc].elemA, nodeTypesDialog[curi->data].name)) {
					old = (HPEN)SelectObject(target, CreatePen(PS_SOLID, 1, DialogColors[dc].elemB));
					SelectObject(target, CreateSolidBrush(DialogColors[dc].elemC));
					break;
				}
			}
			...
			TextOut(target,x-233, y+32+20*i, nodeTypesDialog[curi->data].name, strlen(nodeTypesDialog[curi->data].name));

			if(strlen(nodeTypesDialog[curi->data].helptext)>0){
		...
		LLNode<DialogGraphNode*>* cur = nodes.head;
		while(cur){
			cur->data->x-=vx;
			cur->data->y-=vy;
			cur=cur->next;
		...
		while(cur){
			//if(cur->data->x>=0-cur->data->w && cur->data->x<=w && cur->data->y>=0-cur->data->h && cur->data->y<=h){
				if(cur->data->selected || selgroup.find(cur->data)){
					...
					for(int dc = 0; dc < NUM_DIALOG_COLORS; dc++) {
						if (!strcmp(DialogColors[dc].elemA, cur->data->type->name)) {
							SelectObject(gdc, CreatePen(PS_SOLID,2, DialogColors[dc].elemB));
							SelectObject(gdc, CreateSolidBrush(DialogColors[dc].elemC));
							break;
						}
					}
				}
		...
		if(vy>-h*graphzoom&&vy<h*graphzoom){
			Rectangle(gdc,0,0-vy,w*graphzoom+1,0);
		}

		if(vx>-w*graphzoom&&vx<w*graphzoom){
			Rectangle(gdc,0-vx,0,0,h*graphzoom+1);
		}
		...
		if(menu==1){
			...
			if(!strcmp("Set Portrait",sel->type->name)&&GraphicsLoaded){
				int m=sel->args[0];
			...
			for(i=0;i<4;i++){
				switch(sel->type->argtypes[i]){
					case ARG_CHAR:
						...
						sprintf(out,"%s",CharName[sel->args[i]&(NumItems-1)]);
						TextOut(gdc,w/2+10,dy,out,strlen(out));
					...
					case ARG_ITEM:
						...
						sprintf(out,"%s",ItemName[sel->args[i]&(NumItems-1)]);
						TextOut(gdc,w/2+10,dy,out,strlen(out));
					...
					case ARG_VOICE:
						...
						if (sel->args[i] >= 0 && sel->args[i] < NumGFX[7]) {
							sprintf(out, "%s", SoundNames[MSTextSound[sel->args[i]]+68]);
							TextOut(gdc, w / 2 - 200 + 180, dy, out, strlen(out));
						}
					...
					case ARG_MESSAGE:
						...
						Rectangle(gdc, w / 2 + 40, dy, w / 2 + 102, dy + 20);
						TextOut(gdc, w / 2 + 42, dy + 2, "Edit Text", 9);
		...
		if(menu==2){
			...
			TextOut(gdc, w / 2 - 200 + 40, h / 2 - 150 + 100, "Add Member", 10);
			sprintf(out, "%d", subselect[mode][10]);
			if (focus == 34 && cursor)sprintf(out, "%s|", out);
			TextOut(gdc, w / 2 - 200 + 175, h / 2 - 150 + 100, out, strlen(out));

			Rectangle(gdc, w / 2 - 200 + 258, h / 2 - 150 + 100, w / 2 - 200 + 348, h / 2 - 150 + 119);
			TextOut(gdc, w / 2 - 200 + 287, h / 2 - 150 + 100, "Find", 4);
	...
	void addNode(int x, int y, int t=0, long o=0){
		if(x<0)x=0;
		if(y<0)y=0;
		nodes.add(new DialogGraphNode(x,y,80,80,t,o));
	}
	...
	void click(int px, int py){
				...
				for(int i=0;i<4;i++){
					switch(sel->type->argtypes[i]){
						...
						case ARG_BANKED:
							...
							if (px >= w / 2 + 40 && px <= w / 2 + 102 && py >= dy&&py <= dy + 20) {
								mode = 15;
								if (sel->type->argtypes[i] == ARG_BANKED) {
									subselect[mode][0] = sel->args[i] + sel->bank;
								} else {
									subselect[mode][0] = sel->args[i];
								}
								menu = 0;
								focus = -1;
							}
		...
		if(menu==2){
			...
			if(px>=w/2-200&&px<=w/2+200&&py>=h/2-150&&py<=h/2+150){
				...
				if (px >= w / 2 - 200 + 175 && px <= w / 2 - 200 + 258 && py >= h / 2 - 150 + 100 && py <= h / 2 - 150 + 120) {
					focus = 34;
				}
				if (px >= w / 2 - 200 + 258 && px <= w / 2 - 200 + 348 && py >= h / 2 - 150 + 100 && py <= h / 2 - 150 + 119) {
					search(2);
				}
		...
		while(curi&&i<18){
			...
			if(px>=x-42&&px<=x-22&&py>=y+30+20*i&&py<=y+50+20*i){
				MessageBox(NULL,nodeTypesDialog[curi->data].helptext,"Help",MB_OK);
				return;
			}
	...
	void search(int type) {
		...
		switch (type) {
			...
			case 1:
				...
				break;
			case 2:
				node = findAddMember(subselect[mode][10]);
		...
		if (node) {
			...
			if (searchmulti && subselect[mode][0] != searchstart - 1 && (searchstart != 0 || subselect[mode][0] != NUM_DIALOG - 1)) {
				subselect[mode][0]++;
				if (subselect[mode][0] == NUM_DIALOG)subselect[mode][0] = 0;
			} else {
				searchstart = -1;
	...
	void dclick(int px, int py){
		...
		while(curi&&i<18){
			if(px>=x-238&&px<=x-22&&py>=y+30+20*i&&py<=y+50+20*i){
				addNode(vx+w/2*graphzoom,vy+h/2*graphzoom,curi->data);
				selplace=-1;
				InvalidateRect(hwnd,NULL,0);
				return;
			}
	...
	void connect(DialogGraphNode * a, DialogGraphNode * b, bool prev=false){

		if(a->out.size<a->type->numout){
			a->out.append(b);
			b->in.append(a);
		} else {
	...
	void removeIns() {
		if (!sel)return;
		LLNode<DialogGraphNode*>*cur = sel->in.head;
		while (cur) {
	...
	void removeOuts() {
		if (!sel)return;
		LLNode<DialogGraphNode*>*cur = sel->out.head;
		while (cur) {
	...
	void remove(){
		...
		if (selgroup.size) {
			LLNode<DialogGraphNode*>*cur2 = selgroup.head;
