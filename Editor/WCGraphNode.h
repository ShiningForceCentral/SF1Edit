struct WCGraphNode {
	...
	LinkedList<WCGraphNode*> in;
	LinkedList<WCGraphNode*> out;
	...
	WCGraphNode(int ix, int iy, int iw, int ih, int t = 0, long o = 0) {
		branch = 0;
		brasec = false;
		branch2 = 0;
		brasec2 = false;
		...
		if (!strcmp(type->name, "Run Scripted Movement")) {
			extra = new unsigned char[256];
		}
	}
	...
	void paint(HDC target) {
		...
		LLNode<WCGraphNode*>*cur = out.head;
		i = 0;
		while (cur) {
