#include "BExitGraphNode.h"

#define NUM_BEXIT_COLORS	3
Triple<String, COLORREF, COLORREF> BExitColors[] = {
...
struct BExitGraph {
	LinkedList<BExitGraphNode*> nodes;
	BExitGraphNode * sel;
	int selplace;
	bool scroll;
	bool drag;
	bool dragsel;
	LinkedList<BExitGraphNode*> selgroup;
	...
	BExitGraph(int ix, int iy, int iw, int ih) {
		x = ix;
		y = iy;
		w = iw;
		h = ih;
	...
	void copy() {
		...
		LLNode<BExitGraphNode*>* cur = selgroup.head;
		LLNode<BExitGraphNode*>* cur2;
		LLNode<BExitGraphNode*>* cur3;

		sprintf(out, "EXNodes - %d\r\n", selgroup.size);
		...
		while (cur) {
			type = cur->data->type - nodeTypesBExit;
	...
	void paste() {
		...
		for (int q = 0; q < t; q++) {
			...
			add = new BExitGraphNode(x, y, 80, 80, ot);
			nodes.add(add);
			selgroup.append(add);
			if (!q)sel = add;
	...
	void buildList() {
		choices.clean();
		for (int i = 0;i<NumBExitNodeTypes;i++) {
			if (nodeTypesBExit[i].shape == select[mode])choices.append(i);
		}
	}
	...
	BExitGraphNode * findMessage(int mess, BExitGraphNode * start = 0) {
		BExitGraphNode * ret = 0;
		BExitGraphNode * cur = start;
	...
	BExitGraphNode * findEvent(int event, BExitGraphNode * start = 0) {
		BExitGraphNode * ret = 0;
		BExitGraphNode * cur = start;
		if (!cur)cur = nodes.tail->data;
	...
	BExitGraphNode * findRegion(int region, BExitGraphNode * start = 0) {
		BExitGraphNode * ret = 0;
		BExitGraphNode * cur = start;
		if (!cur)cur = nodes.tail->data;
	...
	BExitGraphNode * findExit(int exit, BExitGraphNode * start = 0, bool first=false) {
		BExitGraphNode * ret = 0;
		BExitGraphNode * cur = start;
		if (!cur)cur = nodes.tail->data;
	...
	void paint(HDC target) {
		...
		while (curi&&i<18) {
			...
			for (int dc = 0; dc < NUM_BEXIT_COLORS; dc++) {
				if (!strcmp(BExitColors[dc].elemA, nodeTypesBExit[curi->data].name)) {
					old = (HPEN)SelectObject(target, CreatePen(PS_SOLID, 1, BExitColors[dc].elemB));
					SelectObject(target, CreateSolidBrush(BExitColors[dc].elemC));
					break;
				}
			}
			...
			TextOut(target, x - 233, y + 32 + 20 * i, nodeTypesBExit[curi->data].name, strlen(nodeTypesBExit[curi->data].name));

			if (strlen(nodeTypesBExit[curi->data].helptext)>0) {
		...
		LLNode<BExitGraphNode*>* cur = nodes.head;
		while (cur) {
			cur->data->x -= vx;
			cur->data->y -= vy;
			cur = cur->next;
		...
		while (cur) {
			...
			if (cur->data->selected || selgroup.find(cur->data)) {
				...
				for (int dc = 0; dc < NUM_BEXIT_COLORS; dc++) {
					if (!strcmp(BExitColors[dc].elemA, cur->data->type->name)) {
						SelectObject(gdc, CreatePen(PS_SOLID, 2, BExitColors[dc].elemB));
						SelectObject(gdc, CreateSolidBrush(BExitColors[dc].elemC));
						break;
					}
				}
			}
	...
	void addNode(int x, int y, int t = 0, long o = 0) {
		if (x<0)x = 0;
		if (y<0)y = 0;
		nodes.add(new BExitGraphNode(x, y, 80, 80, t, o));
	}
		...
		while (curi&&i<18) {
			if (strlen(nodeTypesBExit[curi->data].helptext)>0)
				if (px >= x - 42 && px <= x - 22 && py >= y + 30 + 20 * i&&py <= y + 50 + 20 * i) {
					MessageBox(NULL, nodeTypesBExit[curi->data].helptext, "Help", MB_OK);
					return;
				}
	...
	void search(int type) {
		...
		if (node) {
			...
			if (searchmulti && subselect[mode][0] != searchstart - 1 && (searchstart != 0 || subselect[mode][0] != NUM_BEXIT - 1)) {
				subselect[mode][0]++;
				if (subselect[mode][0] == NUM_BEXIT)subselect[mode][0] = 0;
			} else {
				searchstart = -1;
						...
						if (cur->data!=sel && px >= cur->data->x&&px <= cur->data->x + cur->data->w&&py >= cur->data->y&&py <= cur->data->y + cur->data->h) {
							if (!sel->in.find(cur->data) && cur->data->type->shape != 2 && cur->data->type->shape != 3) {
								...
								if (MessageBox(hwnd, "Connect end to target?", "Confirm", MB_YESNO) == IDYES) {
									...
									LLNode<BExitGraphNode*>*find;
									LLNode<BExitGraphNode*>*ins;
	...
	void connect(BExitGraphNode * a, BExitGraphNode * b, bool prev = false) {
	...
	void removeIns() {
		if (!sel)return;
		LLNode<BExitGraphNode*>*cur = sel->in.head;
		while (cur) {
	...
	void removeOuts() {
		if (!sel)return;
		LLNode<BExitGraphNode*>*cur = sel->out.head;
		while (cur) {
	...
	void remove() {
		...
		if (selgroup.size) {
			LLNode<BExitGraphNode*>*cur2 = selgroup.head;
