...
#define NUM_CS_COLORS	0
Triple<String, COLORREF, COLORREF> CSColors[] = {
...
struct CSGraph {
	LinkedList<CSGraphNode*> nodes;
	CSGraphNode * sel;
	int selplace;
	bool scroll;
	bool drag;
	bool dragsel;
	LinkedList<CSGraphNode*> selgroup;
	...
	CSGraph(int ix, int iy, int iw, int ih) {
		x = ix;
		y = iy;
		w = iw;
		h = ih;
	...
	void copy() {
		...
		LLNode<CSGraphNode*>* cur = selgroup.head;
		LLNode<CSGraphNode*>* cur2;
		LLNode<CSGraphNode*>* cur3;

		sprintf(out, "CSNodes - %d\r\n", selgroup.size);
		...
		while (cur) {
			type = cur->data->type - nodeTypesCS;
	...
	void paste() {
		...
		for (int q = 0; q < t; q++) {
			...
			add = new CSGraphNode(x, y, 80, 80, ot);
			nodes.add(add);
			selgroup.append(add);
			if (!q)sel = add;
	...
	void buildList() {
		choices.clean();
		for (int i = 0; i<NumCSNodeTypes; i++) {
			if (nodeTypesCS[i].shape == select[mode])choices.append(i);
		}
	}
	...
	CSGraphNode * findMessage(int mess, CSGraphNode * start = 0) {
		CSGraphNode * ret = 0;
		CSGraphNode * cur = start;
	...
	CSGraphNode * findEvent(int event, CSGraphNode * start = 0) {
		CSGraphNode * ret = 0;
		CSGraphNode * cur = start;
		if (!cur)cur = nodes.tail->data;
	...
	CSGraphNode * findRegion(int region, CSGraphNode * start = 0) {
		CSGraphNode * ret = 0;
		CSGraphNode * cur = start;
		if (!cur)cur = nodes.tail->data;
	...
	CSGraphNode * findExit(int exit, CSGraphNode * start = 0, bool first = false) {
		CSGraphNode * ret = 0;
		CSGraphNode * cur = start;
		if (!cur)cur = nodes.tail->data;
	...
	void paint(HDC target) {
		...
		while (curi&&i<18) {
			...
			for (int dc = 0; dc < NUM_CS_COLORS; dc++) {
				if (!strcmp(CSColors[dc].elemA, nodeTypesCS[curi->data].name)) {
					old = (HPEN)SelectObject(target, CreatePen(PS_SOLID, 1, CSColors[dc].elemB));
					SelectObject(target, CreateSolidBrush(CSColors[dc].elemC));
					break;
				}
			}
			...
			TextOut(target, x - 233, y + 32 + 20 * i, nodeTypesCS[curi->data].name, strlen(nodeTypesCS[curi->data].name));

			if (strlen(nodeTypesCS[curi->data].helptext)>0) {
		...
		LLNode<CSGraphNode*>* cur = nodes.head;
		while (cur) {
			cur->data->x -= vx;
			cur->data->y -= vy;
			cur = cur->next;
		...
		while (cur) {
			...
			if (cur->data->selected || selgroup.find(cur->data)) {
				...
				for (int dc = 0; dc < NUM_CS_COLORS; dc++) {
					if (!strcmp(CSColors[dc].elemA, cur->data->type->name)) {
						SelectObject(gdc, CreatePen(PS_SOLID, 2, CSColors[dc].elemB));
						SelectObject(gdc, CreateSolidBrush(CSColors[dc].elemC));
						break;
					}
				}
			}
	...
	void addNode(int x, int y, int t = 0, long o = 0) {
		if (x<0)x = 0;
		if (y<0)y = 0;
		nodes.add(new CSGraphNode(x, y, 80, 80, t, o));
	}
		...
		while (curi&&i<18) {
			if (strlen(nodeTypesCS[curi->data].helptext)>0)
				if (px >= x - 42 && px <= x - 22 && py >= y + 30 + 20 * i&&py <= y + 50 + 20 * i) {
					MessageBox(NULL, nodeTypesCS[curi->data].helptext, "Help", MB_OK);
					return;
				}
	...
	void search(int type) {
		...
		if (node) {
			...
			if (searchmulti && subselect[mode][0] != searchstart - 1 && (searchstart != 0 || subselect[mode][0] != NUM_CS - 1)) {
				subselect[mode][0]++;
				if (subselect[mode][0] == NUM_CS)subselect[mode][0] = 0;
			} else {
				searchstart = -1;
						...
						if (cur->data != sel && px >= cur->data->x&&px <= cur->data->x + cur->data->w&&py >= cur->data->y&&py <= cur->data->y + cur->data->h) {
							if (!sel->in.find(cur->data) && cur->data->type->shape != 2 && cur->data->type->shape != 3) {
								...
								if (MessageBox(hwnd, "Connect end to target?", "Confirm", MB_YESNO) == IDYES) {
									...
									LLNode<CSGraphNode*>*find;
									LLNode<CSGraphNode*>*ins;
	...
	void connect(CSGraphNode * a, CSGraphNode * b, bool prev = false) {
	...
	void removeIns() {
		if (!sel)return;
		LLNode<CSGraphNode*>*cur = sel->in.head;
		while (cur) {
	...
	void removeOuts() {
		if (!sel)return;
		LLNode<CSGraphNode*>*cur = sel->out.head;
		while (cur) {
	...
	void remove() {
		...
		if (selgroup.size) {
			LLNode<CSGraphNode*>*cur2 = selgroup.head;
